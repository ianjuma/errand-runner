...
def getAllTasks(username):
    username = username
    return render_template('allTasks.html', username=username)
...
def taskInfo(username, task_id):
    ...
    return render_template('editTask.html', task_category=task_category, task_urgency=task_urgency,
...
@app.route('/api/removeUser/', methods=['DELETE'])
def removeUser():
    if not request.json:
        abort(400)
